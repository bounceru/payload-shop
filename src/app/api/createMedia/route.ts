// File: src/app/api/createMedia/route.ts
...
export async function POST(request: Request) {
  try {
    ...
    const doc = await payload.create({
      ...
      data: {
        tenant,
        // any other fields your media collection requires
      },
      file: {
        data: buffer,
        mimetype: file.type,
    ...
    // 6) Respond with the newly created doc
    return NextResponse.json(doc, { status: 201 })
  } catch (error: any) {
    console.error('Error creating media in custom route:', error)
    return NextResponse.json(
