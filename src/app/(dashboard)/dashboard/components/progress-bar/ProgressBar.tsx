...
const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  const progressClasses = 'absolute left-0 h-full rounded-full'
  ...
  const renderLabel = () => {
    if (label === 'outside') {
      ...
      return (
        <span
          className="absolute inset-0 flex items-center justify-center text-white font-medium text-[10px] leading-tight">
          {progress}%
        </span>
      )
    }
