...
import { ShoppingCart, Check, ChevronLeft, CreditCard, User, Ticket } from "lucide-react";

// Adjust to your real payload types if needed
import type { Customer, Addon } from "@/payload-types";
    ...
    return (
            ...
            <div className="max-w-4xl mx-auto mb-8">
                <div className="flex justify-between">
                    {["Stoelen", "Klantgegevens", "Add-ons", "Betaling"].map((step, index) => (
                        <div key={index} className="flex flex-col items-center">
                            ...
                            <span className={`text-sm font-medium ${currentStep === index + 1 ? "text-blue-600" : "text-gray-500"}`}>{step}</span>
                        </div>
                    ))}
                </div>
                ...
                {timeLeft > 0 && (
                    <p className="text-center text-md text-gray-600 mt-12">
                        Zetels blijven bezet voor {Math.ceil(timeLeft / 60000)} minuten, rond de betaling af om je selectie te behouden.
                    </p>
                )}
            </div>
      ...
      */}
                    ...
                    {selectedSeats.length > 0 && (
                        <div className="md:w-1/3 bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden transition-all">
                            <div className="p-6">
                                    ...
                                    {selectedSeats.map((s) => (
                                        <li key={s.id} className="flex justify-between items-center py-2 border-b border-gray-100">
                                            <span className="text-gray-700">
                                                Rij {s.row} - Zetel {s.seat} <span className="text-gray-400">→</span> {s.ticketType?.name ?? "No Ticket"}
                                            </span>
