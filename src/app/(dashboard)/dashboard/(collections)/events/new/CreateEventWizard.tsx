    ...
    return (
        <div className="max-w-xl mx-auto space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
            ...
            <div className="flex justify-between pt-4 border-t border-gray-100 dark:border-white/[0.05]">
                {step > 1 ? (
                    <button onClick={prevStep} className="rounded-lg border border-gray-300 px-4 py-2 text-sm hover:bg-gray-100 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800">
                        Terug
                    </button>
                ) : (
                    <div />
                ...
                {step < 3 ? (
                    <button onClick={nextStep} className="rounded-lg bg-stagepasssecondary px-4 py-2 text-sm text-white hover:bg-stagepasssecondary-700 hover:scale-105">
                        Volgende
                    </button>
                ) : (
                    <button onClick={handleFinish} className="rounded-lg bg-stagepasssecondary px-4 py-2 text-sm text-white hover:bg-stagepasssecondary-700 hover:scale-105">
                        Afronden
                    </button>
                )}
            </div>
...
function StepOne({ eventData, setEventData }: any) {
    return (
        <div className="space-y-4">
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Einddatum en tijd (optioneel)</label>
                <input
                    type="datetime-local"
                    className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                    value={eventData.end}
